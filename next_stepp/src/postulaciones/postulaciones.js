/**
 * Lógica e Interacción del Módulo de Postulaciones (JobConnect / NextStepp)
 */
...
function updateDashboardStats() {
  if (statTotal) statTotal.textContent = postulaciones.length;
  const interviewed = postulaciones.filter(p => p.id % 3 === 0).length;
  const inReview = postulaciones.filter(p => p.id % 3 === 1).length;
  
  if (statInterviewed) statInterviewed.textContent = interviewed;
